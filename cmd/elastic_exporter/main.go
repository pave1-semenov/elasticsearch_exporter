...
import (
	...
	"iss.digital/mt/elastic_exporter"
	_ "net/http/pprof"
)
...
var (
	...
	configFile    = flag.String("config.file", "elastic_exporter.yml", "ElasticSearch Exporter configuration file name.")
)

func init() {
	prometheus.MustRegister(version.NewCollector("elastic_exporter"))
}
...
func main() {
	...
	if *showVersion {
		fmt.Println(version.Print("elastic_exporter"))
		os.Exit(0)
	}

	log.Infof("Starting ElasticSearch exporter %s %s", version.Info(), version.BuildContext())

	exporter, err := elastic_exporter.NewExporter(*configFile)
	if err != nil {
	...
	http.Handle("/elastic_exporter_metrics", promhttp.Handler())
